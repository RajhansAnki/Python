...
def text_screen(text, color, pos, size):
    font = pygame.font.Font("static_media/font/Inktype-MAp2J.ttf", size)
...
def score_menu():
    ...
    while not exit_score:
        ...
        text_screen(str(hi_score), dark_green, [50, 130], 100)
...
def our_snake(snake_block, snake_list):
    for x in snake_list:
        pygame.draw.rect(root, dark_green, [x[0], x[1], snake_block, snake_block])
        pygame.draw.rect(root, dark_green, [x[0] + 3, x[1] + 3, snake_block - 6, snake_block - 6])
...
def game_loop():
    ...
    while not game_over:
        ...
        text_screen(str(cur_score), dark_green, [5, 0], 60)
